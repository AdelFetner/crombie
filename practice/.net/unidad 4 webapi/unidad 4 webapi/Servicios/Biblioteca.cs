namespace unidad_4_webapi.Servicios
{
...

    {
        public class Biblioteca
        {
            ...
            public Libro DevolverLibro(string isbn, int IUserID)
            {
                ...
                string userID = Console.ReadLine();
                Libro LibroAEncontrar = Libros.Find((libro) => libro.ISBN == userISBN);
                if (!(LibroAEncontrar is null)) {
                    return LibroAEncontrar;
                }
                throw new ArgumentNullException(nameof(LibroAEncontrar));
            }
        }
    }
